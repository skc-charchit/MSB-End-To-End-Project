...
class DataTransformation:
    ...
    def get_data_transformer_object(self):
        ...
        try:
            numerical_columns = [
                'CreditScore', 'FirstPaymentDate', 'MaturityDate', 'MIP', 'Units', 
                'OCLTV', 'DTI', 'OrigUPB', 'LTV', 'OrigInterestRate', 
                'OrigLoanTerm', 'MonthsDelinquent', 'MonthsInRepayment'
            ]
            categorical_columns = [
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            target_column_name = 'EverDelinquent'
            ...
            if input_feature_train_arr.shape[0] != target_feature_train_df.shape[0]:
                raise ValueError("Shape mismatch between input features and target feature in training set")
            if input_feature_test_arr.shape[0] != target_feature_test_df.shape[0]:
                raise ValueError("Shape mismatch between input features and target feature in test set")
            ...
            # Log the shape of the final arrays
            logging.info(f"Shape of final training array: {train_arr.shape}")
            logging.info(f"Shape of final test array: {test_arr.shape}")
